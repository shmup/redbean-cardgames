...
local function argerror(cond, narg, extramsg, name)
	...
	local msg =
		("bad argument #%d to %s%s"):format(
			narg,
			name,
			extramsg and " " .. extramsg or ""
		)
	if not cond then
		error(msg, 3)
	end
...
local function makePath(name, params)
	...
	route = route:gsub(PARAM .. "([^(*:]*)", function(sigil, param, rest)
		...
		return ((params[param] or (params[param] == false and "" or sigil .. param)) .. rest:gsub(
			"^%b[]",
			""
		))
	end)
	...
	argerror(
		not param,
		2,
		"(missing required parameter " .. (param and #param > 0 and param or "splat") .. ")"
	)
	return route
end
...
local function genEnv(opt)
	...
	return function(t, key)
		...
		if opt.request and val == nil and type(key) == "string" then
			local func =
				reqapi[key] or _G["Get" .. key:sub(1, 1):upper() .. key:sub(2)]
...
local function render(name, opt)
	...
	argerror(
		templates[name],
		1,
		"(unknown template name '" .. tostring(name) .. "')"
	)
	...
	for k, v in pairs(rawget(env, ref) or {}) do
		params[k] = v
	end
...
local function setTemplate(name, code, opt)
	...
	argerror(
		ctype == "string" or ctype == "function",
		2,
		"(string, table or function expected)"
	)
	...
	if ctype == "string" then
		...
		argerror(
			tmpl.parser ~= nil,
			2,
			"(referenced template doesn't have a parser)"
		)
	...
	local env = setmetatable(
		...
		tmpl and getmetatable(
			getfenv(tmpl.handler)
		) or (opt or {}).autotag and tmplTagHandlerEnv or tmplRegHandlerEnv
	)
...
local function findset(s)
	return setmap[s] or s:match("%p") and s or error(
...
local function route2regex(route)
	...
	local params = {}
	local regex = route:gsub("%)", "%1?"):gsub("%.", "\\."):gsub(
		PARAM,
		function(sigil, param)
			if sigil == "*" and param == "" then
				param = "splat"
			end
			if sigil == ":" and param == "" then
		...
		-- update optional groups from () to ()?
		-- escape dots (.)
		-- ignore everything that doesn't match `:%w` pattern
		"%b[](%+%))(%b[])([^/:*%[]*)",
		function(sep, pat, rest)
...
local function findRoute(route, opts)
	for i, r in ipairs(routes) do
		...
		if route == r.route and (type(ometh) == "table" and table.concat(
			ometh,
			","
		) or ometh) == (type(rmeth) == "table" and table.concat(
			rmeth,
			","
		) or rmeth) then
			return i
...
local function setRoute(opts, handler)
	...
	elseif ot == "table" then
		if #opts == 0 then
			argerror(false, 1, "(one or more routes expected)")
		end
	else
	...
	argerror(
		ht == "function" or ht == "string" or ht == "nil",
		2,
	...
	if ot == "table" then
		...
		for k, v in pairs(opts) do
			if type(v) == "table" then
				...
				if v.regex then
					v.regex =
						re.compile(v.regex) or argerror(
							false,
							3,
							"(valid regex expected)"
						)
				end
	...
	while true do
		...
		local methods =
			tmethod == "table" and opts.method or tmethod == "string" and {
				opts.method,
			} or { "ANY" }
		LogVerbose(
			"set route '%s' (%s) at index %d",
			route,
...
local function getAllowedMethod(matchedRoutes)
	...
	for _, idx in ipairs(matchedRoutes) do
		...
		if routeMethod then
			for _, method in
				ipairs(
					type(routeMethod) == "table" and routeMethod or {
						routeMethod,
					}
				)
			do
	...
	table.sort(methods)
	return (#methods > 0 and table.concat(
		methods,
		", "
	) .. (methods.OPTIONS == nil and ", OPTIONS" or "") or "GET, HEAD, POST, PUT, DELETE, OPTIONS")
end
						...
						if filter ~= "otherwise" then
							...
							if not res then
								otherwise =
									type(
										cond
									) == "table" and cond.otherwise or opts.otherwise
								LogDebug(
								...
								if otherwise then
									if type(otherwise) == "function" then
										...
										if otherwise == 405 and not req.headers.Allow then
											req.headers.Allow =
												getAllowedMethod(matchedRoutes)
										end
...
local function makeLastModified(asset)
	...
	return {
		function(ifModifiedSince)
			local isModified =
				(not ifModifiedSince or ParseHttpDateTime(
					ifModifiedSince
				) < lastModified)
			if isModified then
				getRequest().headers.LastModified =
					FormatHttpDateTime(lastModified)
			end
			return isModified
		end,
...
local validators = {
	...
	minlen = function(s, num)
		return #tostring(
			s or ""
		) >= num, "%s is shorter than " .. num .. " chars"
	end,
	maxlen = function(s, num)
		return #tostring(
			s or ""
		) <= num, "%s is longer than " .. num .. " chars"
	end,
...
local function makeValidator(rules)
	...
	for i, rule in ipairs(rules) do
		argerror(
			type(rule) == "table",
			1,
			"(table expected at position " .. i .. ")"
		)
		argerror(
			type(rule[1]) == "string",
			1,
			"(rule with name expected at position " .. i .. ")"
		)
		argerror(
							...
							if type(checkname) == "string" then
								...
								if not validator then
									argerror(
										false,
										1,
										"unknown validator " .. checkname
									)
								end
								local success, msg = validator(value, checkval)
								if not success then
									local key =
										rules.key and param or #errors + 1
									local errmsg =
										(err or msg or "%s check failed"):format(
											param
										)
...
local function makeBasicAuth(authtable, opts)
	...
	return {
		function(authorization)
			...
			return authtable[user] == (hash and GetCryptoHash(
				hash:upper(),
				pass,
				key
			) or pass)
...
local function makeIpMatcher(list)
	...
	for _, ip in ipairs(list) do
		...
		mask = tonumber(mask)
		argerror(
			mask and mask >= 0 and mask <= 32,
			1,
			("invalid mask in %s"):format(ip)
		)
		mask = ~0 << (32 - mask)
...
local fm = setmetatable(
	{
		...
		serveError = function(status, reason)
			return function()
				return error2tmpl(status, reason)
			end
		end,
		...
		serveRedirect = function(loc, status)
			return function()
	...
	{ __index = function(t, key)
		...
		if serveStatus then
			return cache(t.serveResponse(tonumber(serveStatus)))
		end
...
local function setSession(session)
	...
	if session and next(session) then
		...
		local sig =
			EncodeBase64(GetCryptoHash(sopts.hash, msg, sopts.secret or ""))
...
local function setHeaders(headers)
	for name, value in pairs(headers or {}) do
		...
		if not noheader or val:lower() == noheader then
			...
			LogDebug(
				"header '%s' with value '%s' is skipped to avoid conflict",
				name,
				val
			)
		end
...
local function hcall(func, ...)
	...
	return false, error2tmpl(
		500,
		nil,
		IsLoopbackIp(GetRemoteAddr()) and err or nil
	)
end
local function handleRequest(path)
	...
	elseif not co then
		LogWarn(
			"unexpected result from action handler: '%s' (%s)",
			tostring(res),
			tres
		)
	end
	-- set the content type returned by the render
	if (type(conttype) == "string" and not rawget(
		req.headers or {},
		"ContentType"
	)) then
	...
	if type(conttype) == "table" then
		...
		for name, value in pairs(conttype) do
...
local function run(opts)
	...
	for key, v in pairs(opts) do
		...
		elseif key:find("Options$") and type(v) == "table" then
			...
			if fm[key] then
				fm[key] = opts[key] -- if there is no default, it's some wrong option
			else
	...
	if sopts.secret == true then
		...
		LogVerbose(
			"applied random session secret; set `fm.sessionOptions.secret`" .. (" to `fm.decodeBase64('%s')` to continue using this value"):format(
				EncodeBase64(sopts.secret)
...
fm.setTemplate("fmt", {
	parser = function(tmpl)
		...
		local tupd = (tmpl .. "{%" .. EOT .. "%}"):gsub(
			"(.-){%%([=&]*)%s*(.-)%s*%%}",
			function(htm, pref, val)
				return writer(
					htm
				) .. (val ~= EOT and (pref == "" and val .. " " or ("write(%s(tostring(%s or '')))"):format( -- this is not the suffix -- this is a code fragment
			...
			local function writeAttrs(opt)
				for attrname, attrval in pairs(opt) do
					if type(attrname) == "string" then
						...
						local escape =
							not (valtype == "table" and attrval[1] == "raw")
						...
						if type(attrval) == "string" or type(
							attrval
						) == "number" then
							if escape then
...
tests = function()
	...
	if not isRedbean then
		re = { compile = function(exp)
			return { search = function(self, path)
				local res = { path:match(exp) }
				if #res > 0 then
					table.insert(res, 1, path)
				end
				return unpack(res)
			end }
		end }
		EscapeHtml = function(s)
			return (string.gsub(s, "&", "&amp;"):gsub('"', "&quot;"):gsub(
				"<",
				"&lt;"
			):gsub(">", "&gt;"):gsub("'", "&#39;"))
		end
		ParseIp = function(str)
			...
			return (v1 and (tonumber(v1) << 24) + (tonumber(
				v2
			) << 16) + (tonumber(v3) << 8) + tonumber(v4) or -1) -- match ParseIp logic in redbean
		end
	...
	IsLoopbackIp = function()
		return true
	end
	GetRemoteAddr = function() end
	GetHttpReason = function(status)
		return tostring(status) .. " reason"
	end
	Log = function(_, ...)
		print("#", ...)
	end
	...
	local section = ""
	local function outformat(s)
		return type(s) == "string" and ("%q"):format(s):gsub(
			"\n",
			"n"
		) or tostring(s)
	end
	...
	local function done()
		print(("1..%d # Passed %d/%d"):format(num, success, num))
	end
	...
	fm.setTemplate(
		tmpl1,
		"Hello, {% for i, v in ipairs({3,2,1}) do %}-{%= v %}{% end %}"
	)
	fm.render(tmpl1)
	...
	do
		fm.setTemplate(tmpl2, [[{a: "{%= title %}"}]], {
			title = "set when adding template",
		})
		fm.render(tmpl2)
		is(
			out,
			'{a: "set when adding template"}',
			"JSON with value set when adding template"
		)
		...
		is(
			out,
			'{a: "set from render"}',
			"JSON with a passed value set at rendering"
		)
		...
		fm.setTemplate(
			tmpl2,
			[[{% local title = "set from template" %}{a: "{%= title %}"}]]
		)
		fm.render(tmpl2)
	...
	fm.setTemplate(tmpl2, [[{a: "{%= title %}"}]], {
		title = "set when adding",
	})
	fm.setTemplate(tmpl1, "Hello, {% render('tmpl2') %}")
	fm.render(tmpl1)
	is(
		out,
		[[Hello, {a: "set when adding"}]],
		"`include` other template with a local value"
	)
	...
	is(
		out,
		[[Hello, {a: "value"}]],
		"`include` other template with passed value set at rendering"
	)
	...
	is(
		err:match('string "Hello, World!'),
		'string "Hello, World!',
		"error references original template code"
	)
	...
	fm.setTemplate(tmpl1, "Hello, {% main() %}World!", {
		main = function() end,
	})
	fm.render(tmpl1)
	is(
		out,
		[[Hello, World!]],
		"used function can be passed when adding template"
	)

	rt({
		GetZipPaths = function()
			return { "/views/hello1.fmt", "/views/hello2.fmg" }
		end,
		LoadAsset = function(s)
			return ({
		...
		function()
			...
			is(
				out,
				[[Hello, value 1]],
				"rendered default template loaded from an asset"
			)
			...
			is(
				err:match("unknown template name"),
				"unknown template name",
				"only specified extensions loaded from an asset"
			)
			...
			is(
				out,
				[[Hello, value 1]],
				"rendered default template loaded from an asset with folder name"
			)
	...
	is(
		route2regex("/foo/:bar_none/"),
		"^/foo/([^/]+)/$",
		"route with a named parameter with underscore"
	)
	is(
		route2regex("/foo(/:bar)"),
		"^/foo(/([^/]+))?$",
		"route with a named optional parameter"
	)
	is(
		route2regex("/foo/:bar[%d]"),
		"^/foo/([0-9]+)$",
		"route with a named parameter and a customer set"
	)
	is(
		route2regex("/foo/:bar[^%d]"),
		"^/foo/([^0-9]+)$",
		"route with a named parameter and not-in-set"
	)
	is(
		route2regex("/foo/:bar[]5]"),
		"^/foo/([]5]+)$",
		"route with a starting closing bracket in a set"
	)
	is(
		route2regex("/foo/:bar[]5].:some"),
		"^/foo/([]5]+)\\.([^/]+)$",
		"route with a closing bracket in a set followed by another parameter"
	)
	is(
		route2regex("/foo/:bar[^]5]"),
		"^/foo/([^]5]+)$",
		"route with a starting closing bracket in not-in-set"
	)
	is(
		route2regex("/foo/:bar[1%]2%-%+]"),
		"^/foo/([1[.].]2[.-.]+]+)$",
		"route with a closed bracked"
	)
	is(
		route2regex("/foo(/:bar(/:more))"),
		"^/foo(/([^/]+)(/([^/]+))?)?$",
		"route with two named optional parameters"
	)
	is(
		route2regex("/foo(/:bar)/*.zip"),
		"^/foo(/([^/]+))?/(.*)\\.zip$",
		"route with an optional parameter and a splat"
	)
	is(
		route2regex("/foo(/:bar)/*splat.zip"),
		"^/foo(/([^/]+))?/(.*)\\.zip$",
		"route with an optional parameter and a named splat"
	)
	is(
		select(2, pcall(route2regex, "/foo/:bar[%o]")):match(": (.+)"),
	...
	is(
		routes[routes["/foo/bar"]].handler,
		nil,
		"assign no handler to a static route"
	)
	fm.setRoute(fm.PUT"/foo/bar")
	is(
		routes["/foo/bar"],
		index + 1,
		"route with the same name and different method is added"
	)
	...
	matchRoute("/foo/some/123.myext/mo/re.zip", {
		params = {},
	})
	fm.setRoute(route, function(r)
		is(
			r.params.bar,
			"some.myext",
			"[1/4] default optional parameter matches dots"
		)
		is(
			not r.params.more,
			true,
			"[2/4] missing optional parameter gets `false` value"
		)
		is(
			not r.params.ext,
			true,
			"[3/4] missing optional parameter gets `false` value"
		)
		is(r.params.splat, "more", "[4/4] splat matches")
	end)
	matchRoute("/foo/some.myext/more.zip", {
		params = {},
	})
	if isRedbean then
		local called = false
		fm.setRoute(route, function()
			called = true
		end)
		matchRoute("/foo/some.myext/more", {
			params = {},
		})
	...
	do
		...
		GetAssetMode = function(m)
			return m:find("/$") and 2 ^ 14 or 2 ^ 15
		end

		local path
		RoutePath = function(s)
			path = s
			return s ~= nil
		end
		fm.setRoute("/*path", "/asset/*path")
		...
		is(
			path,
			nil,
			"Directory is not returned for empty parameter with internal routing"
		)
		...
		is(
			path,
			"/foo/some.myext/more.lua",
			"Forwarded path is returned for internal routing"
		)
	...
	is(
		headerMap.CacheControl,
		"Cache-Control",
		"Cache-Control header is mapped"
	)
	...
	is(
		matchCondition("GET", "GET"),
		true,
		"attribute matches based on simple value"
	)
	is(
		matchCondition("GET", { GET = true }),
		true,
		"attribute matches based on simple value in condition table"
	)
	is(matchCondition("GET", {}), false, "non-existing attribute doesn't match")
	is(
		matchCondition(nil, "GET"),
		false,
		"`nil` value doesn't match a simple value"
	)
	is(
		matchCondition(nil, { GET = true }),
		true,
		"`nil` value matches a value in condition table"
	)
	is(
		matchCondition("GET", {
			GET = true,
	...
	is(
		matchCondition("text/html; charset=utf-8", {
			regex = re.compile("text/"),
		}),
		true,
		"attribute matches based on regex"
	)
	is(
		matchCondition("text/html; charset=utf-8", {
			pattern = "%f[%w]text/html%f[%W]",
		}),
		true,
		"attribute matches based on Lua pattern"
	)
	is(
		matchCondition("GET", "POST"),
		false,
		"attribute doesn't match another simple value"
	)
	is(
		matchCondition("GET", { POST = true }),
		false,
		"attribute doesn't match if not present in condition table"
	)
	is(
		matchCondition("text/html; charset=utf-8", {
			regex = re.compile("text/plain"),
		}),
		false,
		"attribute doesn't match another regex"
	)
	is(
		matchCondition(nil, function()
			return true
		end),
		true,
		"`nil` value matches function that return `true`"
	)
	is(
		matchCondition(nil, function()
			return false
		...
		false,
		"`nil` value doesn't match function that return `false`"
	)
	is(
		matchCondition("GET", function()
			return true
		end),
		true,
		"attribute matches function that return `true`"
	)
	is(
		matchCondition("GET", function()
			return false
		...
		false,
		"attribute doesn't match function that return `false`"
	)
	is(
		matchCondition("GET", { function()
			return true
	...
	is(
		routes[routes.acceptencoding].options.AcceptEncoding.pattern,
		"%f[%w]gzip%f[%W]",
		"known header generates pattern-based match"
	)
	...
	is(
		proute.more,
		"parameters",
		"POST method on a table preserves existing conditions"
	)
	...
	do
		...
		is(
			header,
			"Content-Type",
			"custom template with options sets Content-Type"
		)
		is(
			value,
			"application/json",
			"custom template with options sets expected Content-Type"
		)
		...
		is(
			header,
			"Content-Type",
			"preset template with options sets Content-Type"
		)
		is(
			value,
			"application/json",
			"preset template with options sets expected Content-Type"
		)

		fm.setRoute(
			"/",
			fm.serveContent("html", { { "h1", "Title" }, {
				"div",
				a = 1,
				{
					"p",
		...
		is(
			out,
			[[<h1>Title</h1><div a="1"><p checked="checked">text</p></div>]],
		...
		is(
			out,
			"<!doctype html><body><h1>post title</h1>&lt;!&gt;<!-- --></body>" .. '<div hx-post="url"></div><script>a<b</script><p>text</p>' .. '<table style="b<a"><tr><td>3</td><td>4</td><td>12</td></tr></table>' .. "<table>more</table><p>1</p><br/><p>3</p><p>2</p><p>1</p>" .. '<div a="&quot;1&#39;"><p>text+{a: "T"}</p></div>' .. "<iframe><p>1</p><p>2</p><p>3</p></iframe>",
			"preset template with html generation"
		)
		...
		fm.setRoute(
			"/",
			fm.serveResponse(200, { ContentType = "text/html" }, "text")
		)
		handleRequest()
		is(
			value,
			"text/html",
			"explicitly set content-type takes precedence over auto-detected one"
		)
		...
		is(
			routes.routeOne,
			routes["/route1"],
			"first route (our of several) gets name assigned"
		)
	end
	...
	do
		...
		is(
			cookie,
			"MyCookie",
			"Deleted cookie is processed when set to `false` (value)"
		)
		...
		if isRedbean then
			...
			is(
				value,
				"e2E9IiJ9.SHA256.lua.AYDGTB6O7W4ohlbpRtgvY2NiDFUdS1efkd0ZpROoL+Q="
			)
		end
	end
	...
	fm.setRoute("/", function(r)
		...
		is(
			type(r.formatHttpDateTime),
			"function",
			"formatHttpDateTime function is available"
		)
	end)
	if isRedbean then
		handleRequest()
	...
	do
		...
		is(
			rname,
			"foobar",
			"duplicate route with the same routeName triggers warning"
		)
	end
	is(routes.foobar, routes[route], "route name can be used as alias")
	is(
		routes[routes.foobar].routeName,
		nil,
		"route name is removed from conditions"
	)

	_, err = pcall(fm.makePath, route)
	is(
		err:match("missing required parameter splat"),
		"missing required parameter splat",
		"required splat is checked"
	)
	_, err = pcall(fm.makePath, "/foo/:bar")
	is(
		err:match("missing required parameter bar"),
		"missing required parameter bar",
		"required parameter is checked"
	)
	is(
		fm.makePath(route, { splat = "name" }),
		"/foo/name.zip",
		"required splat is filled in"
	)
	is(
		fm.makePath("/assets/*asset", { asset = false }),
		"/assets/",
		"empty splat is filled in"
	)
	is(
		fm.makePath("/foo/*more/*splat.zip", {
			more = "some",
			splat = "name",
		}),
		"/foo/some/name.zip",
	...
	is(
		fm.makePath("foobar", { splat = "name" }),
		makePath(route, { splat = "name" }),
	...
	is(
		...
		"/foo/some/name.zip",
		"single optional parameter is filled in"
	)
	is(
		fm.makePath(route, {
			splat = "name",
		...
		"/foo/some/12.json/name.zip",
		"multiple optional parameters are filled in"
	)
	is(
		fm.makePath("/foo/:bar", { bar = "more" }),
		"/foo/more",
		"unregistered route is handled"
	)
	is(fm.makePath("/foo(/*.zip)"), "/foo", "optional splat is not required")
	is(
		fm.makePath("/foo(/*.zip)", { splat = "more" }),
		"/foo/more.zip",
		"optional splat is filled in"
	)
	...
	is(
		fm.makePath("http://some.website.com:8080/:foo?param=:bar", {
			foo = "some",
			bar = 123,
		}),
		"http://some.website.com:8080/some?param=123",
	...
	if isRedbean then
		local url =
			"http://domain.com/path/more/name.ext?param1=val1&param2=val2#frag"
		GetUrl = function()
			return url
		end
		...
		is(
			makeUrl({ path = "/short" }),
			url:gsub("/path/more/name.ext", "/short"),
			"makeUrl uses path"
		)
		is(
			makeUrl({ scheme = "https" }),
			url:gsub("http:", "https:"),
			"makeUrl uses scheme"
		)
		is(
			makeUrl({ fragment = "newfrag" }),
			url:gsub("#frag", "#newfrag"),
			"makeUrl uses fragment"
		)
		is(
			makeUrl({ fragment = false }),
			url:gsub("#frag", ""),
			"makeUrl removes fragment"
		)
		is(
			makeUrl("", {
				path = "/path",
		...
		is(
			...
			"?a=1&b=&c&d%5B1%5D%5Bname%5D=file",
			"makeUrl generates query string from hash table"
		)
		...
		is(
			out,
			[[Hello, http://domain.com/%3Csome&amp;/path%3E?param1=val1&amp;param2=val2#frag]],
	...
	is(
		out,
		"Server Error: Access forbidden",
		"serveError used as a route handler"
	)
	...
	fm.setRoute("/status", function()
		return fm.serveError(402)
	end)
	handleRequest()
	...
	is(
		rawget(fm, "serve401"),
		fm.serve401,
		"serve401 is cached after first use"
	)
	...
	is(
		status,
		404,
		"not matched condition triggers configured otherwise processing"
	)
	...
	fm.setRoute(
		{
			...
			method = {
				"SOME",
				otherwise = fm.serveResponse(405),
			},
		},
		fm.serve402
	)
	...
	is(
		status,
		405,
		"not matched condition triggers dynamic otherwise processing"
	)
	...
	is(
		getRequest().headers.Allow,
		"GET, OPTIONS",
		"Allow header is returned along with 405 status"
	)
	...
	is(
		...
		"Allow header includes HEAD when 405 status is returned"
	)
	...
	do
		...
		is(
			loc,
			GetPath(),
			"serveRedirect without parameters uses current path as location"
		)
	end
	...
	is(
		out,
		"123-789",
		"route parameter takes precedence over URL parameter with the same name"
	)
	...
	is(
		type(validator[1]),
		"function",
		"makeValidator returns table with a filter handler"
	)
	is(
		type(validator.otherwise),
		"function",
		"makeValidator return table with an 'otherwise' handler"
	)
	...
	is(
		msg.name,
		"Invalid name format",
		"error message is keyed on parameter name"
	)
	is(
		msg.pass,
		"pass is shorter than 5 chars",
		"multiple error message are provided when `all=true` is set"
	)
	...
	is(
		err.name,
		"Invalid name format",
		"scalar parameters get their name from the first rule"
	)
	...
	is(
		res.notcalled,
		true,
		"route action not executed after a failed validator check"
	)
	is(
		type(res.errors),
		"table",
		"failed validator check triggers `otherwise` processing"
	)
	is(
		res.errors[1],
		"bar is shorter than 5 chars",
		"`otherwise` processing gets the list of errors"
	)
	...
	is(
		type(res[1]),
		"function",
		"makeBasicAuth returns table with a filter handler"
	)
	is(
		type(res.otherwise),
		"function",
		"makeBasicAuth returns table with an 'otherwise' handler"
	)
	...
	for n, test in ipairs(matcherTests) do
		...
		is(
			makeIpMatcher(mask)(ParseIp(ip)),
			res,
			("makeIpMatcher %s (%d/%d)"):format(ip, n, #matcherTests)
		)
	end

	local privateMatcher =
		makeIpMatcher({ "192.168.0.0/16", "172.16.0.0/12", "10.0.0.0/8" })
	for val, res in
		pairs({
			["192.168.0.1"] = true,
			["172.16.0.1"] = true,
	...
	if isRedbean then
		...
		is(
			type(fm.isLoopbackIp),
			"function",
			"isLoopbackIp function is available"
		)
		is(type(fm.formatIp), "function", "formatIp function is available")
		is(
			type(fm.formatHttpDateTime),
			"function",
			"formatHttpDateTime function is available"
		)
	end
	...
	GetRedbeanVersion = function()
		return 0x020103
	end
	ProgramBrand = function(b)
		brand = b
	end
	ProgramPort = function(p)
		port = p
	end
	ProgramAddr = function(a)
		addr = addr .. "-" .. a
	end
	ProgramHeader = function(h, v)
		header, value = h, v
	end
	...
	run{
		port = 8081,
		addr = { "abc", "def" },
		headers = { RetryAfter = "bar" },
	}
	is(
		brand:match("redbean/[.%d]+"),
		"redbean/2.1.3",
		"brand captured server version"
	)
	...
	is(
		header .. ":" .. value,
		"Retry-After:bar",
		"default headers set when passed"
	)
	...
	is(
		err:match("unknown option"),
		"unknown option",
		"run reports unknown option"
	)
